...
Фронтендът използва React Router за дефиниране на маршрути и навигация без презареждане на страницата. Всеки URL се свързва към съответен компонент от директорията `pages/`. За управление на състоянието се прилага локален `useState` и `useEffect` hooks, като за по-големи обекти (например списък от бележки) може да се използва Context API.

Комуникацията с бекенда е обвързана в `services/notesService.js`, където са имплементирани функции за извличане, създаване, редактиране и изтриване на бележки. Всички заявки се изпращат с `fetch` API или с библиотека axios, а отговорите се обработват асинхронно.
