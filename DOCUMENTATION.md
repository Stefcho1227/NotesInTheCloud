...
**Spring Boot Starter Security** е модул, който осигурява необходимите зависимости за работа със Spring Security. Той включва библиотеки за управление на автентикация и авторизация, което гарантира, че само упълномощени потребители могат да достъпват определени ресурси.
...
За HTTP заявки се използва предварително конфигуриран **Axios** екземпляр (axiosConfig.js). Той автоматично добавя access token в `Authorization` заглавката и обработва грешки тип 401 чрез опит за рефреш на токена.
...
2. Gradle за управление на зависимости и билд на Java проекта.
...
Можем да достъпим и модифицираме базата данни, изградена в докера със заявката:

```bash
docker exec -it notes-db mariadb -u root -p
```
